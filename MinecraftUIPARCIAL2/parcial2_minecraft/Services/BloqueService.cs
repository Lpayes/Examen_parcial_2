    ...
    public class BloqueService
    {
        ...
        public void Crear(Bloque bloque)
        {
            try
            {
                ...
                connection.Open();
                var command = new SqlCommand("INSERT INTO Bloques (Nombre, Tipo, Rareza) VALUES (@Nombre, @Tipo, @Rareza); SELECT SCOPE_IDENTITY();",
                    connection);
            ...
            {
                throw new Exception($"Error al crear bloque: {ex.Message}");
            }
        }
        ...
        public List<Bloque> ObtenerTodos()
        {
            ...
            {
                throw new Exception($"Error al obtener bloques: {ex.Message}");
            }
            return bloques;
        }

        // Método para buscar bloques por tipo
        public List<Bloque> BuscarPorTipo(string tipo)
        {
            ...
            try
            {
                ...
                var command = new SqlCommand(
                    "SELECT Id, Nombre, Tipo, Rareza FROM Bloques WHERE Tipo LIKE @Tipo", connection);
            ...
            {
                throw new Exception($"Error al buscar bloques por tipo: {ex.Message}");
            }
            return bloques;
        }

        // Método para buscar bloques por rareza
        public List<Bloque> BuscarPorRareza(string rareza)
        {
            ...
            try
            {
                ...
                connection.Open();
                var command = new SqlCommand(
                    "SELECT Id, Nombre, Tipo, Rareza FROM Bloques WHERE Rareza LIKE @Rareza", connection);
            ...
            {
                throw new Exception($"Error al buscar bloques por rareza: {ex.Message}");
            }
            return bloques;
        }
        ...
        public Bloque ObtenerPorId(int id)
        {
            try
            {
                ...
                connection.Open();
                var command = new SqlCommand(
                    "SELECT Id, Nombre, Tipo, Rareza FROM Bloques WHERE Id = @Id", connection);
            ...
            {
                throw new Exception($"Error al obtener bloque: {ex.Message}");
            }
            return null;
        }
        ...
        public void Actualizar(Bloque bloque)
        {
            try
            {
                ...
                connection.Open();
                var command = new SqlCommand(
                    "UPDATE Bloques SET Nombre = @Nombre, Tipo = @Tipo, Rareza = @Rareza WHERE Id = @Id", connection);
                ...
                if (rowsAffected == 0)
                {
                    throw new Exception("No se encontró el bloque para actualizar.");
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"Error al actualizar bloque: {ex.Message}");
            }
        }
        ...
        public void Eliminar(int id)
        {
            try
            {
                ...
                var checkCommand = new SqlCommand(
                    "SELECT COUNT(*) FROM Inventario WHERE BloqueId = @Id", connection);
                ...
                {
                    throw new Exception(
                        $"No se puede eliminar el bloque porque está presente en el inventario de jugadores. Hay {inventoryCount} registros asociados.");
                }
                ...
                if (rowsAffected == 0)
                {
                    throw new Exception("No se encontró el bloque para eliminar.");
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"Error al eliminar bloque: {ex.Message}");
            }
        }
    }
}